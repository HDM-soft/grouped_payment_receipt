def action_generate_grouped_payment_receipt(self):
    ...
    if len(partner_ids) > 1:
        raise UserError("Por favor, selecciona facturas del mismo cliente o proveedor.")

    # Obtener las líneas reconciliadas de las facturas
    reconciled_lines = self.line_ids.filtered(lambda l: l.reconciled)
    if not reconciled_lines:
        raise UserError("No hay líneas reconciliadas en las facturas seleccionadas.")

    # Buscar las reconciliaciones parciales para obtener las líneas de pago
    ...
    # Obtener las líneas de pago desde las reconciliaciones
    ...
    # Obtener los pagos asociados a esas líneas
    payment_ids = (
